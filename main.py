...
def run_simulation():
    # ---------------- USER INPUTS ----------------
    sources = np.array([[5, 2, 1.5]], dtype=np.float32) # multiple sources [[],[],...]
    ...
    ground_on = True
    receivers = np.array([[4, 1, 1.5],[4, 2, 1.5]], dtype=np.float32)
    planes_Z = np.array([1.5, 3.0])  # Z planes on which computing and plotting
    grid_size = (200, 200, len(planes_Z))
    bounds = (0, 10, 0, 10, 0, max(planes_Z)) #xmin,xmax,ymin,ymax,zmin,zmax
